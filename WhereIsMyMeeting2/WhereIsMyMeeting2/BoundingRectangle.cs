...
namespace WhereIsMyMeeting2
{
    class BoundingRectangle
    {
        ...
        public static BoundingRectangle GetBoundsFromPath(List<PathInfo> list)
        {
            ...
            if (list.First().segmentType != PathInfo.SegmentType.Move)
            {
                return null;
            }
            if (list.Last().segmentType != PathInfo.SegmentType.Close)
            {
                return null;
            }
            if (list.Skip(1).Take(list.Count - 2).Any(p => p.segmentType != PathInfo.SegmentType.Line))
            {
                return null;
            }
